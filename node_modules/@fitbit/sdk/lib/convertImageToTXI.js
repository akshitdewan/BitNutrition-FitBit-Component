...
function pickOutputFormat(image) {
    ...
    return image_codec_txi_1.TXIOutputFormat.RGBA8888;
}
function transformPNG(png) {
            ...
            png.on('parsed', () => {
                try {
                    const txi = image_codec_txi_1.encode({
                        ...
                        outputFormat: pickOutputFormat(metadata),
                        rle: 'auto',
                    });
...
function transformPNGBuffer(file) {
    const png = new pngjs_1.PNG();
    const transformer = transformPNG(png);
    png.parse(file);
    return transformer;
}
function transformPNGStream(file) {
    let started = false;
...
function convertImageToTXI() {
    const stream = new stream_1.Transform({
        ...
        transform(file, _, cb) {
            ...
            file.basename += '.txi';
            if (file.isBuffer()) {
                transformPNGBuffer(file.contents)
                    .then((txi) => {
                    file.contents = txi;
                    cb(undefined, file);
                })
                ...
                file.contents = transformPNGStream(file.contents)
                    .on('error', err => this.emit('error', new plugin_error_1.default(PLUGIN_NAME, err, { fileName })));
                cb(undefined, file);
            }
        },
    });
